...
class FSRS6 {
  ...
  nextDifficulty(oldD, rating) {
    const g      = rating + 1;
    const target = this._easyInitialDifficultyUnclamped();
    const delta  = -this.w[6] * (g - 3);
    const newD   = oldD + delta * (10 - oldD) / 9;
  ...
  shortTermStability(oldS, rating) {
    ...
    const stabPart = Math.pow(oldS, -this.w[19]);
    let multiplier = expPart * stabPart;

    // TEST CHANGE: Removed the cap for GOOD (rating 2) so it increases on same-day reviews
    if (rating === GRADES.EASY) {
      multiplier = Math.max(multiplier, 1.0);
    }
    // GOOD now bypasses the cap and can increase
