...
/* ─── ONE-TIME MIGRATION ─────────────────────────────────────────────────────
  ...
  function getAllCardsForDate(ds) {
    ...
    state.topics.forEach(t => {
      let include = false;
      if (t.isPastFixed) {
        include = t.startDate === ds ||
          (Array.isArray(t.fixedDates) && t.fixedDates.includes(ds));
      } else if (t.startDate && t.startDate > today) {
        include = t.startDate === ds;
      } else {
        include = getCardNextReviewDate(state.sm2[t.id]) === ds;
      }
      if (include) {
        seen.add(t.id);
        out.push(t);
    ...
    // Merge in any topic that was actually reviewed on this date,
    // sourced from state.reviewLog (per-day rating events).
    const log = state.reviewLog && state.reviewLog[ds];
    if (Array.isArray(log)) {
      log.forEach(entry => {
        if (!entry || !entry.cardId || seen.has(entry.cardId)) return;
        const t = state.topics.find(x => x.id === entry.cardId);
        if (t) {
          seen.add(t.id);
          out.push(t);
        }
  ...
  function renderCalGrid() {
    ...
    // Also mark every date that has historical review activity (gray dot)
    if (state.reviewLog && typeof state.reviewLog === 'object') {
      Object.keys(state.reviewLog).forEach(d => {
        const arr = state.reviewLog[d];
        if (Array.isArray(arr) && arr.length) {
          const uniq = new Set(arr.map(e => e && e.cardId).filter(Boolean));
          pastMap[d] = (pastMap[d] || 0) + uniq.size;
        }
      });
    }
  ...
  function onCalClick(ds) {
    ...
    renderCalGrid();
    renderCalPanel();
    // On mobile, slide the side panel up as a bottom-sheet drawer so the
    // user can reach the "+ Add Topic" button.
    if (typeof window.openCalDrawer === 'function') window.openCalDrawer();
  }
  ...
  function renderCalPanel() {
    ...
    cards.forEach(t => {
      ...
      const thumbHtml = t.image ? `<img src="${t.image}" alt="" style="height:64px;width:96px;object-fit:cover;border-radius:6px;margin-right:12px;float:left">` : '';
      div.innerHTML = `
        ${thumbHtml}
  ...
  function renderBrowse() {
    ...
    const activeFilter = (filterEl && filterEl.value) || T.browseDeckFilter || 'all';
    const isDue        = activeFilter === 'due';
  
    const titleEl = el('browseDeckTitle');
    if (titleEl) titleEl.textContent = `Browse: ${deck.name}${isDue ? ' — Due Only' : ''}`;
  
    const subEl = document.querySelector('#section-browse .sec-sub');
    if (subEl) {
      subEl.innerHTML = isDue
        ? `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="vertical-align:-1px;margin-right:4px"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>View only — showing ${cards.length} due card${cards.length !== 1 ? 's' : ''}`
        : `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="vertical-align:-1px;margin-right:4px"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>View only — no changes to your schedule`;
    }
  ...
  function runViewSession(containerId, cards, onDone, doneLabel, deckOverride = null) {
    ...
    function attachKey() {
      ...
      keyHandler = e => {
        ...
        // Enter or → to advance when answer is shown
        if (answerShown && (e.key === 'Enter' || e.key === 'ArrowRight')) {
          e.preventDefault();
    ...
    function render() {
      ...
      const nextLabel = idx === cards.length - 1 ? 'Done' : 'Next';

      const controls = !answerShown
        ? `<div class="fc-show-row" style="display:flex; justify-content:center; width:100%;"><button class="btn-show-answer" id="vsShowBtn">Show Answer &nbsp;<kbd>Space</kbd></button></div>`
        : `<div class="fc-show-row" style="display:flex; justify-content:center; width:100%;">
            <button class="btn-show-answer" id="vsNextBtn">${nextLabel} &nbsp;<kbd>Enter</kbd></button>
          </div>`;

      const c = el(containerId);
      if (!c) { cleanup(); return; }

      // Cloze vs standard formatting
      let renderedQ = '';
      let renderedA = '';
      const qTitle  = t.title || t.question || '';
      const qAnswer = t.content || t.answer || '';

      if (t.type === 'cloze') {
        renderedQ = typeof renderClozeQ === 'function' ? renderClozeQ(qTitle) : esc(qTitle);
        const clozeAns = typeof renderClozeA === 'function' ? renderClozeA(qTitle) : esc(qTitle);
        renderedA = clozeAns + (qAnswer ? `<div style="margin-top:14px;font-size:0.9em;font-weight:400;color:var(--ink2)">${esc(qAnswer)}</div>` : '');
      } else {
        renderedQ = esc(qTitle);
        renderedA = qAnswer ? esc(qAnswer) : '<span style="color:var(--ink3);font-style:italic">No additional notes</span>';
      }

      // First 30 chars preview
      const qText = qTitle.replace(/\{\{c\d+::([^}:]+)(?:::[^}]+)?\}\}/g, '$1').trim();
      const qTruncated = qText.length > 30 ? qText.slice(0, 30) + '...' : qText;
      const qPreviewHtml = answerShown
        ? `<div class="fc-q-preview" aria-hidden="true">${esc(qTruncated)}</div>`
        : '';

      c.innerHTML = `
        <div class="fc-session dv-session">
          <div class="fc-topbar">
            <span class="fc-prog">${idx + 1} / ${cards.length}</span>
            <div class="fc-pb-track" style="flex:1; margin:0 12px;">
              <div class="fc-pb-fill" style="width:${(idx / cards.length) * 100}%"></div>
            </div>
          </div>
          <div class="fc-card dv-card" id="dvCard" aria-live="polite" data-flag="${t.flag || ''}" data-suspended="${t.suspended ? 'true' : 'false'}">
            <div class="dv-card-meta" style="width:100%; display:flex; gap:10px; margin-bottom:12px;">
              <span class="dv-deck-tag" style="background:${deck.color}22;color:${deck.color}">${esc(deck.name)}</span>
            </div>
            <div class="fc-q" id="dvQ">${renderedQ}</div>
            <div class="fc-answer-area ${answerShown ? '' : 'hidden'}" id="dvAnswerArea">
              ${qPreviewHtml}
              <div class="fc-answer-sep">— Answer —</div>
              <div class="fc-a" id="dvA">${renderedA}</div>
              <img id="dvCardImage" class="fc-card-image hidden" alt="Card image">
            </div>
            <div class="fc-card-flag-display" id="dvCardFlagDisplay"></div>
          </div>
          <div class="dv-controls" style="margin-top:16px; width:100%; display:flex; justify-content:center;">${controls}</div>
        </div>`;

      // Render KaTeX and rich media elements
      const dvQ = el('dvQ');
      const dvA = el('dvA');
      if (typeof fcEnhanceCardEl === 'function') {
        fcEnhanceCardEl(dvQ);
        fcEnhanceCardEl(dvA);
      }

      // Render image if present
      const dvImg = el('dvCardImage');
      if (dvImg) {
        if (t.image) {
          dvImg.src = t.image;
          dvImg.classList.remove('hidden');
        } else {
          dvImg.classList.add('hidden');
        }
      }

      // Render flag display SVG
      const flagDisplayEl = el('dvCardFlagDisplay');
      if (flagDisplayEl) {
        const flag = t.flag || '';
        if (flag) {
          const color = { red: '#ef4444', orange: '#f97316', green: '#22c55e', blue: '#3b82f6' }[flag] || '#ef4444';
  ...
  function renderJournal() {
    // Auto-select today on first entry so the date head and editor
    // immediately show the current date on both mobile and desktop.
    if (!state.jSelected) {
      state.jSelected = todayStr();
    }
    renderJMiniCal();
    renderJDateStrip();
    renderJEditor();
  ...
  // Mobile horizontal date strip — populates #journalDateStrip with the
  // days of the currently viewed month. Highlights today, the selected
  // day, and any day that already has a journal entry.
  function renderJDateStrip() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      if (ds === state.jSelected) { btn.classList.add('selected'); btn.setAttribute('aria-selected','true'); selectedBtn = btn; }
      if (hasEntry)               btn.classList.add('has-entry');
      if (ds === today)           todayBtn = btn;

      btn.innerHTML = `<span class="jds-dow">${dowNames[date.getDay()]}</span><span class="jds-day">${d}</span>`;
    ...
    // Scroll the selected (or today) chip into view so user immediately
    // sees where they are in the month.
    const focusBtn = selectedBtn || todayBtn;
    if (focusBtn && typeof focusBtn.scrollIntoView === 'function') {
      try { focusBtn.scrollIntoView({ block: 'nearest', inline: 'center' }); } catch {}
    }
  }
